...
MeshNode::MeshNode(const std::shared_ptr<Mesh> mesh)
	: m_mesh(mesh), m_model_matrix(glm::identity<glm::mat4>()),
	m_normal_local_to_world_matrix(glm::identity<glm::mat4>()) {}

MeshNode::MeshNode(const glm::mat4 model_matrix)
	: m_mesh(std::make_shared<Mesh>()), m_model_matrix(model_matrix),
...
void MeshNode::draw(Uniforms &uniforms) const {
	for (auto & mesh_section : m_mesh->sections) {
		...
		mesh_section.material->uniforms.set_mat3("normal_local_to_world_matrix", m_normal_local_to_world_matrix);
